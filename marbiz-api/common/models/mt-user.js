...
module.exports = function (Mtuser) {
    ...
    Mtuser.sendphoneverificationCode = sendPhoneVerificationCode;
    Mtuser.sendOTP = sendOTP;
    Mtuser.verifyOTP = verifyOTP;
    Mtuser.testSMS = testSMS;
    Mtuser.resetUserPassword = resetUserPassword;
    Mtuser.resetUserPassFromPanel = resetUserPassFromPanel;
    Mtuser.changeUserStatus = changeUserStatus;
    Mtuser.generateAccessToken = generateAccessToken;
    // admin.initializeApp({
    ...
    function sendOTP(handle, mode, cb) {
        if (mode == 'phone') {
            Mtuser.findOne({
                ...
                Mtuser.sendphoneverificationCode(userObj, function (err, data) {
                    cb(null, data);
                });
            });
        } else if (mode == 'email') {
            Mtuser.findOne({
                ...
                if (err) cb(null, err);
                if (!userObj) cb('User record not found', null);
                Mtuser.sendEmailOtp(userObj, function (err, data) {
                    cb(null, data);
                });
            });
        } else {
    ...
    function generateAccessToken(username, cb) {
        ...
        try {
            AccessToken.createAccessTokenId((e, token) => {
                Mtuser.findOne({ where: { username } }).then(user => {
                    AccessToken.create({
                        id: token,
                        userId: user.id,
                        ttl: 1209600,
